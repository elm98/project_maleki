		...
		get:function (action) {
			return $.get(action).done(function (r) {
				...
				if(e.responseJSON.hasOwnProperty('errors')){
					...
					const params = new Proxy(new URLSearchParams('?'+($split.length > 1?$split[1]:'')), {
						get: (searchParams, prop) => searchParams.get(prop),
					});
					if(params.showError === 'no'){
						return 0;
					}
					helper().toast({
						text:'هنگام بازیابی اطلاعات از سرور خطایی رخ ' +
							'' +
							'داده دوباره تلاش کنید',
						type:'error'
					});
					return 0;
				}
