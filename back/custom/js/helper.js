...
$(document).ready(function () {
	...
	$('.modal').modal();
	helper().focusAll();
	$('table').on('click','.dt-row-delete',function () {
		let $this=$(this),
			$tr = $this.closest('tr'),
			$id = 0,
			find=$tr.find('input.dt-row-checkbox');
		if(!find.length){
			let prevTr = $($tr).prev()[0];
			find = $(prevTr).find('input.dt-row-checkbox');
			if(find.length){
				$id=$(find[0]).val();
			}
		}else{
		...
		if($id > 0){
			helper().one_row_delete(this,$id,$this.data('action'));
		}
	});
	$(".pdate").formatter({pattern:"{{9999}}/{{99}}/{{99}}"});
});
	...
	return{
		...
		post:function (action,data) {
			...
			return $.post(action,data).done(function (r) {
				...
				.fail(function (e) {
					let txt='هنگام دریافت اطلاعات از سرور خطایی رخ داده دوباره تلاش کنید';
					if(e.responseJSON.hasOwnProperty('errors')){
						for(let row in e.responseJSON.errors){
							e.responseJSON.errors[row].forEach(function(v) {
								txt = v;
		...
		check_all:function(){
			$(".check-all").on('click',function () {
				var $children=$(this).closest('table').find(".dt-row-checkbox");
				if($(this).prop('checked')){
					$children.prop('checked',true);
					$children.closest('tr').css('background','#e5a9ff');
				}else{
					$children.prop('checked',false);
					$children.closest('tr').css('background','inherit');
				}
			});
			$('table').on('click','input[type="checkbox"].dt-row-checkbox',function () {
				if($(this).prop('checked')){
					$(this).closest('tr').css('background','#e5a9ff');
				}else{
					$(this).closest('tr').css('background','inherit');
				}
			});
		},
		item_delete:function(action,id,func){
			swal({
				...
				if(e){
					let foo = [id];
					helper().post(action,{foo:foo}).done(function (r) {
		...
		rows_delete:function(action){
			var $arr=[];
			var $list=$('input[type=checkbox].dt-row-checkbox:checked');
			$list.each(function(){
				if($(this).prop('checked'))
				{
					$arr.push($(this).val());
			...
			this.confirm_delete(action,{arr:$arr,list:$list});
		},
		one_row_delete:function(th,id,action){
			var obj={arr:[id],list:$(th)};
			this.confirm_delete(action,obj);
		},
		confirm_delete:function (action,obj) {
			let arr=obj.arr;
			let list=obj.list;
			if(arr.length < 1){
				swal({
			...
			swal({
				...
				if(e){
					helper().post(action,{foo:arr}).done(function (r) {
						if(r.hasOwnProperty('result') && r.result){
							list.closest('tr').each(function () {
								if($(this).hasClass('child')){
									let prevRow = $( this ).prev()[0];
									console.log($(prevRow));
									$(prevRow).remove();
								}
								$(this).remove();
		...
		selectAjax:function(){
			$(".select2_ajax").each(function(){
				...
				let parent=typeof $this.data('parent')!=='undefined'?$("#"+$this.data('parent')):'';
				let $field=typeof $this.data('field') != "undefined"?$this.data('field'):'id';
				$this.select2({
					dropdownParent:parent,
					dropdownAutoWidth: !0,
					width: "100%",
		...
		loadingOn:function ($id) {

		},
		loadingOff:function ($id) {

		},
		number2persian:function (number,id) {
			return $("#"+id).html(Num2persian(number));
		},
		focusAll:function () {
			$("body").on('focus','input.focus-all',function () {
				$(this).select();
			});
		},
