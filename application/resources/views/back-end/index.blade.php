...
@section('headerScript')
    <!--Chart Js--->
    <link rel="stylesheet" type="text/css" href="{{$dotSlashes}}back/app-assets/vendors/chartist-js/chartist.min.css">
    <link rel="stylesheet" type="text/css" href="{{$dotSlashes}}back/app-assets/vendors/chartist-js/chartist-plugin-tooltip.css">
    <link rel="stylesheet" type="text/css" href="{{$dotSlashes}}back/app-assets/css-rtl/pages/dashboard-modern.css">
    <link rel="stylesheet" type="text/css" href="{{$dotSlashes}}back/app-assets/css-rtl/pages/intro.min.css">
    <!--Clock Analog--->
    <link href="{{_slash('plugin/clock-analog')}}/style.css" rel="stylesheet" type="text/css">
    <script src="{{_slash('plugin/clock-analog')}}/prefixfree.min.js" type="cf9533af10c137594dc1687f-text/javascript"></script>
            ...
            <div class="card subscriber-list-card animate fadeUp" style="width: 100%">
                ...
                <table class="subscription-table responsive-table highlight">
                    ...
                    @foreach($list as $item)
                        <tr>
                            <td>{!! imageTitle($item->img,$item->title,30) !!}</td>
                            <td><span> {!! status_color($item->status,\App\Models\Post::status($item->status)) !!} </span></td>
                            <td><a href="{{url('/management/post-edit/'.hashId($item->id))}}" class="btn btn-link">نمایش</a></td>
                        </tr>
                    @endforeach
                    ...
                    <div class="col s12 m12">
                        ...
                        <div>
                            <p style="color: #000;text-align: center">
                                <span>امروز</span>
                                <span>{{vv(date('Y-m-d'),'l , j F Y')}}</span>
                            </p>
                            <p style="color: #000;text-align: center">
                                <span>Today</span>
            ...
            <div class="card recent-buyers-card animate fadeUp" style="width: 100%">
                <div class="card-content">
                    ...
                    <ul class="collection mb-0">
                        ...
                        <li class="collection-item avatar">
                            <img src="{{_slash('uploads/avatar/'.$user->avatar)}}" onerror="this.src='{{_slash('back/custom/img/avatar.png')}}'" alt="" class="circle" />
                            <p class="font-weight-600">{{$user->nickname}}</p>
                            <p class="medium-small">{{vv($user->created_at,'j F Y')}}</p>
                            <a href="./customer-edit/{{hashId($user->id)}}" class="secondary-content"><i class="material-icons">link</i></a>
                        </li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class="card-content">
                    ...
                    <div class=" mt-8">
                        <div>
                            @if($list->count())
                            <p class="login-chip">
                                {!! imageTitle('uploads/avatar/'.$list[0]->avatar,$list[0]->nickname) !!}
                                <hr/>
                            </p>
                            @endif
                            @foreach($list as $item)
                                <p>
                                    <span class="chip font-small indigo lighten-4 black-text width-100">{{vv($item->created_at)}}</span>
                                </p>
                            @endforeach
                        </div>
                    </div>
...
@section('footerScript')
    ...
    <!--Clock Analog--->
    <script src="{{_slash('plugin/clock-analog')}}/index.js" type="cf9533af10c137594dc1687f-text/javascript"></script>
    <script src="{{_slash('plugin/clock-analog')}}/rocket-loader.min.js" data-cf-settings="cf9533af10c137594dc1687f-|49" defer=""></script>

    <!--Chart Js--->
    <script src="{{$dotSlashes}}back/app-assets/vendors/chartjs/chart.min.js"></script>
    <script src="{{$dotSlashes}}back/app-assets/vendors/chartist-js/chartist.min.js"></script>
    <script src="{{$dotSlashes}}back/app-assets/vendors/chartist-js/chartist-plugin-tooltip.js"></script>
    <script src="{{$dotSlashes}}back/app-assets/vendors/chartist-js/chartist-plugin-fill-donut.min.js"></script>

    {{--<script src="{{$dotSlashes}}back/app-assets/js/scripts/intro.min.js"></script>--}}
    <script>
        $(".card-animate").hover(
            function () {
