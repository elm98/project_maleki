    ...
    <div class="container" id="app_vue_body">
        <div class="auth-wrapper">
            <div class="auth-form shadow-around mt-3">
                ...
                <div method="post" action="{{url('/login-done')}}">
                    {{csrf_field()}}
                ...
                <div class="auth-form-footer">
                    ...
                    <a href="{{url('/register?callback='.$callback)}}" class="link--with-border-bottom font-small"> اینجا کلیک کنید</a>
					<span class="font-small">همچنین میتوانید</span>
					<a href="{{url('/login?callback='.$callback)}}" class="link--with-border-bottom font-small"> با رمز عبور</a>
            ...
            vm = new Vue({
                ...
                methods:{
                    ...
                    sendMobile(){
                        ...
                        vm.error_mesage = '';
                        vm.loading = 1;
                        helper().post('{{url("/")}}/mobile-verify-code',{
                            mobile:vm.mobile,
                        })
                        .done(function (r) {
                            if(r.result){
                    ...
                    checkAndLogin(){
                        vm.error_mesage = '';
                        vm.loading = 1;
                        helper().post('{{url("/")}}/mobile-check-code',{
                            mobile:vm.mobile,
                            verify_code:vm.verify_code,
                        })
                        .done(function (r) {
                            if(r.result){
                                window.location.href = vm.callback.length?vm.callback.length:'{{url("/")}}';
                            }else{
